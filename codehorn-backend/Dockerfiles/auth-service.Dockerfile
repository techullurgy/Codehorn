...
RUN chmod +x ./gradlew \
    ...
    && apt update \
    && apt install unzip -y \
    && unzip /app/codehorn/auth-service.jar -d temp
...
RUN $JAVA_HOME/bin/jdeps \
      ...
      /app/codehorn/auth-service.jar > ./jre-modules.txt \
    && $JAVA_HOME/bin/jlink \
      --verbose \
...
COPY --from=jre-build /app/codehorn/auth-service.jar /app/auth-service.jar

RUN apt-get update && apt-get install curl -y

EXPOSE 80

CMD ["java", "-Dserver.port=80", "-jar", "/app/auth-service.jar"]
