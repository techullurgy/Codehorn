...
RUN chmod +x ./gradlew \
    ...
    && mv /app/codehorn/gateway-service/build/libs/gateway-service-${CODEHORN_APP_VERSION}.jar /app/codehorn/gateway-service.jar \
    && apt update \
    && apt install unzip -y \
    && unzip /app/codehorn/gateway-service.jar -d temp
...
RUN $JAVA_HOME/bin/jdeps \
      ...
      /app/codehorn/gateway-service.jar > ./jre-modules.txt \
    && $JAVA_HOME/bin/jlink \
      --verbose \
